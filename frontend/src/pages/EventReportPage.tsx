...
import { eventsAPI, routersAPI, settingsAPI, type EventReport, type EventClassificationRule } from '../services/api';
...
export default function EventReportPage() {
  ...
  const [classifications, setClassifications] = useState<EventClassificationRule[]>([]);
  const [eventType, setEventType] = useState('');
  const [chartMode, setChartMode] = useState<'daily' | 'average'>('daily');
  const reportRef = useRef<HTMLDivElement>(null);

  useEffect(() => { routersAPI.list().then(setRouters).catch(() => {}); settingsAPI.eventClassificationRules().then(r => setClassifications(r.rules.filter(rule => rule.enabled))).catch(() => {}); }, []);
  const load = async () => {
    if (!routerId) return toast.error('Seleccioná un router');
    try { setReport(await eventsAPI.report(routerId, from || undefined, to || undefined, eventType || undefined)); }
